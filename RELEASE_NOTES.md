...
* Углублённый трек по Docker поверх 05, 06 и 09. «Docker 01 · Рантайм»: hardening контейнера (пользователь, `privileged`, сокет Docker, capabilities, `no-new-privileges`, read-only корень и tmpfs, лимиты, seccomp, секреты файлом) и сегментация сетей compose. «Docker 02 · Поставка»: база по digest, зависимости с хешами, секреты BuildKit, SBOM, подпись cosign, гейт допуска, rootless BuildKit и шаблон workflow
* Руководство «Dockerfile: как писать правильно»; в руководстве по окружению описаны режимы сети VirtualBox
* Шпаргалки: новые по Linux и сетям и по Nmap; Git и GitHub CLI переписаны с пояснениями; у остальных пояснён каждый блок; исправлен синтаксис примеров `.gitignore` и `.dockerignore`
* Справочники: «CVSS и реестр рисков», «Разбор находок сканеров», «Как читать схемы курса»; порты переписаны в карточки «сервис, риск, проверка, защита»; дополнены классификация инструментов и лицензии
* Troubleshooting: схема диагностики
* Лабораторные: восстановлены уязвимые стенды и исправлены инструкции; единые названия работ, опечатки и формулировки в материалах, лабах и тестах
* У каждого блока кода есть метка языка или имя файла
* Рамка поиска с анимированным кольцом, набор favicon, шапка во всю ширину; уведомление об ответственности перенесено в политику конфиденциальности

**Схемы:**

* Схемы перестроены как встроенный Mermaid в нотации ГОСТ 19.701-90 с ортогональными линиями
* Схема работы есть в каждой лабораторной и в каждом руководстве
* Mermaid размещён на сайте и загружается по требованию: схемы рисуются и при переходах без перезагрузки страницы

**SEO:**

* Заголовки страниц без повторного названия сайта и не длиннее 65 символов, описания не длиннее 160
* Один `h1` на странице: скрытая шапка README с бейджами больше не попадает в HTML лабораторных
* Структурированные данные `LearningResource` на страницах лабораторных; обновлён `llms.txt`

**Архитектура и сборка:**

* Редиректы переехавших страниц делает собственный код в `hooks.py` вместо плагина `mkdocs-redirects`
* README: маршрут курса, схемы сборки и доставки, актуальное дерево и счётчики
* `.gitignore`: конфликт-копии iCloud; удалён каталог `specs`
...
* Адаптив главной ярусами 1600 / 1220 / 960 / 700 px
...
* CSS разделён на `components.css` и `home.css` (главная грузит свои стили сама), почищены мёртвые правила
