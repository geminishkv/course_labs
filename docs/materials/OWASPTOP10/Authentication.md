...
Раздел, посвященный аутентификации описывает атаки, направленные на используемые Web-приложением методы проверки идентификатора пользователя, службы или приложения. Аутентификация использует как минимум один из трёх механизмов (факторов): "что-то, что мы имеем", "что-то, что мы знаем" или "что-то, что мы есть". В этом разделе описываются атаки, направленные на обход или эксплуатацию уязвимостей в механизмах реализации аутентификации Web-серверов.
...
flowchart TB
    accTitle: Ход атаки: подбор учётных данных
    accDescr: Нарушитель: список логинов и паролей. Затем: автоматический перебор формы входа. Если защита на месте (есть лимит попыток, блокировка и второй фактор), подбор остановлен. Если защиты нет, пара логин и пароль подобрана; итог: вход от имени пользователя.

    start_actor(["Нарушитель: список<br/>логинов и паролей"])
    step_one["Автоматический перебор<br/>формы входа"]
    control_gate{"Есть лимит попыток,<br/>блокировка и второй<br/>фактор?"}
    control_fork((" "))
    attack_stopped(["Подбор остановлен"])
    impact_step["Пара логин и пароль<br/>подобрана"]
    attack_result(["Итог: вход от имени<br/>пользователя"])
...
%%{init: {"flowchart": {"curve": "step"}}}%%
flowchart TB
    accTitle: Ход атаки: недостаточная аутентификация
    accDescr: Нарушитель: адрес закрытого раздела. Затем: прямой запрос к разделу в обход формы входа. Если защита на месте (сервер проверяет сессию на каждом запросе), запрос отклонён: нужен вход. Если защиты нет, раздел отдан без проверки личности; итог: доступ к закрытым функциям.

    start_actor(["Нарушитель: адрес<br/>закрытого раздела"])
    step_one["Прямой запрос к<br/>разделу в обход формы<br/>входа"]
    control_gate{"Сервер проверяет<br/>сессию на каждом<br/>запросе?"}
    control_fork((" "))
    attack_stopped(["Запрос отклонён: нужен<br/>вход"])
    impact_step["Раздел отдан без<br/>проверки личности"]
    attack_result(["Итог: доступ к<br/>закрытым функциям"])
...
%%{init: {"flowchart": {"curve": "step"}}}%%
flowchart TB
    accTitle: Ход атаки: небезопасное восстановление пароля
    accDescr: Нарушитель: логин жертвы. Затем: запрос восстановления пароля; ответ на слабый вопрос или угаданный код. Если защита на месте (сброс идёт одноразовой ссылкой на почту), сброс не удался. Если защиты нет, пароль жертвы заменён; итог: захват учётной записи.

    start_actor(["Нарушитель: логин<br/>жертвы"])
    step_one["Запрос восстановления<br/>пароля"]
    step_two["Ответ на слабый вопрос<br/>или угаданный код"]
    control_gate{"Сброс идёт одноразовой<br/>ссылкой на почту?"}
    control_fork((" "))
    attack_stopped(["Сброс не удался"])
    impact_step["Пароль жертвы заменён"]
    attack_result(["Итог: захват учётной<br/>записи"])
...
Примером реализации подобной функции является использование "секретного вопроса", ответ на который указывается в процессе регистрации. Вопрос либо выбирается из списка или вводится самим пользователем. Ещё один механизм позволяет пользователю указать "подсказку", которая поможет ему вспомнить пароль. Другие способы требуют от пользователя указать часть персональных данных, таких как номер соц. страхования, ИНН, домашний адрес, почтовый индекс и т.д., которые затем будут использоваться для установления личности.
