...
flowchart TB
    accTitle: Ход атаки: индексирование директорий
    accDescr: Нарушитель: адрес каталога без индексного файла. Затем: запрос каталога. Если защита на месте (листинг каталогов выключен), ответ 403 или 404. Если защиты нет, сервер показывает список файлов; итог: найдены копии и служебные файлы.

    start_actor(["Нарушитель: адрес<br/>каталога без<br/>индексного файла"])
    step_one["Запрос каталога"]
    control_gate{"Листинг каталогов<br/>выключен?"}
    control_fork((" "))
    attack_stopped(["Ответ 403 или 404"])
    impact_step["Сервер показывает<br/>список файлов"]
    attack_result(["Итог: найдены копии и<br/>служебные файлы"])
...
- список пользователей сервера (довольно часто для каждого из пользователей создается папка с именем, основанным на названии учётной записи)
...
%%{init: {"flowchart": {"curve": "step"}}}%%
flowchart TB
    accTitle: Ход атаки: идентификация приложений
    accDescr: Нарушитель: запросы к серверу. Затем: сбор заголовков, страниц ошибок и особенностей ответов. Если защита на месте (версии скрыты, ошибки обезличены), стек остаётся неизвестным. Если защиты нет, определены продукт и версия; итог: подбор известных уязвимостей под версию.

    start_actor(["Нарушитель: запросы к<br/>серверу"])
    step_one["Сбор заголовков,<br/>страниц ошибок и<br/>особенностей ответов"]
    control_gate{"Версии скрыты, ошибки<br/>обезличены?"}
    control_fork((" "))
    attack_stopped(["Стек остаётся<br/>неизвестным"])
    impact_step["Определены продукт и<br/>версия"]
    attack_result(["Итог: подбор известных<br/>уязвимостей под версию"])
...
Существуют семантические особенности при генерации заголовков различными серверами. Например, Apache передаёт параметр `Date` перед значением заголовка `Server`, в то время как IIS использует обратный порядок. При обработке запроса `OPTIONS` Apache возвращает только параметр `Allow`, в то время как IIS дополнительно включает параметр `Public`.
...
%%{init: {"flowchart": {"curve": "step"}}}%%
flowchart TB
    accTitle: Ход атаки: утечка информации
    accDescr: Нарушитель: обычные и ошибочные запросы. Затем: чтение комментариев, ошибок и служебных ответов. Если защита на месте (отладка выключена, лишнее вычищено), в ответах только нужное пользователю. Если защиты нет, видны пути, запросы и внутренние адреса; итог: карта системы для следующей атаки.

    start_actor(["Нарушитель: обычные и<br/>ошибочные запросы"])
    step_one["Чтение комментариев,<br/>ошибок и служебных<br/>ответов"]
    control_gate{"Отладка выключена,<br/>лишнее вычищено?"}
    control_fork((" "))
    attack_stopped(["В ответах только<br/>нужное пользователю"])
    impact_step["Видны пути, запросы и<br/>внутренние адреса"]
    attack_result(["Итог: карта системы<br/>для следующей атаки"])
...
Подробные сообщения об ошибках могут возникать в результате специально сформированного запроса. Ниже приведён пример сообщения при вводе символа апострофа в качестве имени пользователя:
...
%%{init: {"flowchart": {"curve": "step"}}}%%
flowchart TB
    accTitle: Ход атаки: обратный путь в директориях
    accDescr: Нарушитель: имя файла с переходом вверх. Затем: приложение подставляет ввод в путь к файлу. Если защита на месте (путь приводится к каноническому и сверяется), запрос вне каталога отклонён. Если защиты нет, открыт файл вне каталога сайта; итог: чтение конфигурации и исходного кода.

    start_actor(["Нарушитель: имя файла<br/>с переходом вверх"])
    step_one["Приложение подставляет<br/>ввод в путь к файлу"]
    control_gate{"Путь приводится к<br/>каноническому и<br/>сверяется?"}
    control_fork((" "))
    attack_stopped(["Запрос вне каталога<br/>отклонён"])
    impact_step["Открыт файл вне<br/>каталога сайта"]
    attack_result(["Итог: чтение<br/>конфигурации и<br/>исходного кода"])
...
Даже если Web-сервер ограничивает доступ к файлам определённым каталогом, эта уязвимость может возникать в сценариях или CGI-программах. Довольно часто к имени запрашиваемого файла добавляются специальные символы, такие как `%00`, с целью обхода фильтров.
...
В приведённом примере атакующий использует символы `../` для перехода на уровень выше по дереву каталогов. Символ `%00` используется для обхода проверки расширения файла (приложение позволяет обращаться только к файлам `.txt`) и для того, чтобы расширение не использовалось при загрузке файла.
...
%%{init: {"flowchart": {"curve": "step"}}}%%
flowchart TB
    accTitle: Ход атаки: предсказуемое расположение ресурсов
    accDescr: Нарушитель: словарь типовых имён. Затем: перебор адресов: копии, панели, старые версии. Если защита на месте (лишнее удалено, доступ закрыт авторизацией), ответы 404 и 403. Если защиты нет, найден ресурс без ссылок на него; итог: доступ к копиям и служебным панелям.

    start_actor(["Нарушитель: словарь<br/>типовых имён"])
    step_one["Перебор адресов:<br/>копии, панели, старые<br/>версии"]
    control_gate{"Лишнее удалено, доступ<br/>закрыт авторизацией?"}
    control_fork((" "))
    attack_stopped(["Ответы 404 и 403"])
    impact_step["Найден ресурс без<br/>ссылок на него"]
    attack_result(["Итог: доступ к копиям<br/>и служебным панелям"])
