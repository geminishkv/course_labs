...
hide:
  - toc
title: "CheatSheet: команды Docker | Курс AppSec"
...
Образ — неизменяемый шаблон, контейнер — его запущенный экземпляр. Тег (`python:3.12-slim`) — переставляемая метка: для воспроизводимой сборки образ закрепляют дайджестом. `docker history` показывает слои и команды, которыми они созданы, — это первое место, где ищут забытый в образе секрет. Как пишется Dockerfile — в руководстве [Dockerfile: как устроен и как его писать](../guides/dockerfile_guide.md).
...
`docker run` — это создание и запуск одной командой; флаги задают, что контейнеру можно. `-p` публикует порт наружу (без него сервис доступен только из сети Docker), `-v` даёт доступ к файлам хоста, `--rm` убирает контейнер после остановки. Состояния контейнера и переходы между ними — в схеме жизненного цикла в [Основах Docker](../guides/docker_basics.md).
...
!!! warning "Правило"
    Три флага отменяют изоляцию контейнера: `--privileged` (все возможности ядра), `-v /var/run/docker.sock:/var/run/docker.sock` (управление демоном, то есть root на хосте) и `--network host` (сеть хоста без фильтра). В лабораторных они встречаются только как пример того, что находит аудит в Лаб. 06.

## Выполнение команд и логи

Диагностика начинается с логов, а не с входа внутрь: `docker logs` показывает всё, что приложение написало в stdout и stderr. `docker exec` запускает дополнительный процесс в уже работающем контейнере — если контейнер упал, зайти в него нельзя, смотрите `docker logs` и `docker inspect`.
...
Compose описывает несколько контейнеров, их сети и тома в одном файле — так устроены стенды Лаб. 05–09. Команда `docker compose` (через пробел) — актуальная версия v2; `docker-compose` через дефис — снятая с поддержки v1. `down` удаляет контейнеры и сети, но не тома: данные базы переживают `down` и исчезают только с `--volumes`.
...
Контейнеры в одной пользовательской сети видят друг друга по имени сервиса — IP-адреса запоминать не нужно. Том хранит данные отдельно от контейнера и переживает его удаление.
...
Docker ничего не удаляет сам: остановленные контейнеры, старые образы и кэш сборки копятся, пока не кончится диск. Сначала `docker system df`, чтобы увидеть, что занимает место, потом чистка.
...
!!! warning "Правило"
    `docker system prune -a --volumes` удаляет тома вместе с данными баз и все образы, которые сейчас не используются: следующая сборка скачает их заново. На машине со стендами лабораторных сначала `docker compose down` в нужном каталоге, а не общая чистка.
