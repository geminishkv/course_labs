...
Структуру в YAML задают отступы, и только пробелами: табуляция — синтаксическая ошибка. Сколько пробелов на уровень — не важно; важно, чтобы у элементов одного уровня отступ совпадал.
...
Коллекций два вида: список (строки с дефисом) и словарь (пары «ключ: значение»). Почти любой конфиг — это их вложение друг в друга.
...
Каждый уровень вложенности — это отступ. Ошибка на один пробел не ломает разбор, а молча переносит ключ на другой уровень: файл остаётся валидным, но значит уже другое.
...
`|` сохраняет переносы строк — так пишут скрипты в `run:`. `>` склеивает строки в одну — так пишут длинные описания. Перепутанный символ превращает скрипт из пяти команд в одну длинную.
...
Якорь `&` запоминает фрагмент, алиас `*` подставляет его в другом месте. Поддержка зависит от инструмента, который читает файл, — проверяйте результат его собственной командой, например `docker compose config`.
...
YAML сам угадывает тип значения, и угадывает не всегда так, как вы ждёте: `no`, `on`, `yes` во многих парсерах становятся булевыми, `1.10` — числом 1.1, `012` — восьмеричным числом. Всё, что должно остаться строкой, — версии, коды стран, идентификаторы — берите в кавычки.
...
Workflow — это словарь: `on` (когда запускать), `permissions` (что разрешено токену), `jobs` (что делать). Как это работает — в руководстве [Введение в CI/CD](../guides/cicd_basics.md), как писать безопасно — в [шпаргалке GitHub Actions Security](CHEATSHEET_GH_ACTIONS_SECURITY.md).
...
Файл Compose — словарь сервисов, сетей и томов. Итоговую конфигурацию после подстановки переменных и якорей показывает `docker compose config` — смотрите её, прежде чем искать ошибку в самом файле.
...
Правило Semgrep — элемент списка `rules` с обязательными `id`, `message`, `severity`, `languages` и шаблоном. Практика — в [Лаб. 07](../../labs/basic/lab07.md).
...
Ошибка в отступе не всегда даёт ошибку разбора — иногда получается валидный YAML с другим смыслом. Поэтому проверок две: синтаксис (`yamllint`) и смысл — инструментом, который этот файл читает.
...
Ошибки, на которых чаще всего теряют время: файл выглядит правильно, а конвейер падает или, хуже, молча делает не то.
