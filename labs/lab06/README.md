...
В ходе выполнения лабораторной работы были выявлены уязвимости на уровне:
* конфигурации Docker Compose (.yml);
* контейнерных образов;
* параметров запуска контейнеров;
* секретов и учетных данных;
* конфигурации хоста Docker.

Основные причины возникновения уязвимостей:
* Небезопасные параметры контейнеров:
    * privileged: true
    * network_mode: host
    * pid: host
    * cap_add: ALL
    * отключение AppArmor и Seccomp

Эти параметры снимают стандартную контейнерную изоляцию.

* Хардкод секретов:
    * пароли и ключи (ADMIN_PASSWORD, APP_SECRET_KEY, FLAG) передаются через переменные окружения в открытом виде.

* Использование устаревших и небезопасных образов:
    * alpine 3.8 (EOL, без обновлений безопасности);
    * отсутствие pin-версий образов (nginx:latest).
* Монтирование критических директорий хоста:
    * /
    * /var/run/docker.sock
* Отсутствие принципа наименьших привилегий:
    * запуск контейнеров от root (0:0).

- [ ] 6. Опишите влияния уязвимостей, их сценарий атаки

* Сценарий CR (Container Runtime / Privilege Escalation)
...
Злоумышленник получает доступ к контейнеру (RCE, shell, уязвимость в веб-приложении).

Через примонтированный /hostroot он получает доступ к файловой системе хоста.

Возможна:
* модификация системных файлов;
* установка бэкдоров;
* эскалация привилегий до root на хосте.

Влияние:
* Полный компромисс хоста (Critical).
